...
def launch_chrome(
    ...
    try:
        ...
        if proxy:
            ...
            # Use environment variables for proxy authentication (like Indigo/Dolphin)
            if username and password:
                ...
                env["http_proxy"] = proxy_url
                env["https_proxy"] = proxy_url
                log.info("Set proxy auth via environment variables: %s://%s:%s", scheme, host, port)
            else:
                log.info("Using proxy without authentication: %s://%s:%s", scheme, host, port)

            args.append(f"--proxy-server={scheme}://{host}:{port}")
        else:
            log.info("Launching Chrome without proxy")

        # No extensions needed - using environment variables for proxy auth
