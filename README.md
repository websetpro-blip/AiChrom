...
- **Локальная прокси-обёртка (pproxy)**: автоматическая авторизация без диалогов логина (как в Indigo/Octo/GoLogin).
- **Fallback на прямое подключение**: если прокси недоступен, Chrome запускается без прокси.
...
### ERR_TIMED_OUT или диалоги логина
- **Прокси недоступен**: система автоматически переключается на прямое подключение
- **Проверка прокси**: убедитесь, что прокси-сервер доступен
- **Тест без прокси**: запустите `python test_direct_connection.py` для проверки
- **Логи**: проверьте `logs/launcher.log` и `logs/pproxy_*.log`

### Запуск через ярлык
- Ярлык `C:\Users\webse\OneDrive\Desktop\AiChrome.lnk` должен указывать на `StartAiChrome.bat`
- Рабочая директория: `C:\AI\AiChrome`
